...
#---------------------------------------------
    ...
    # ROC curve
    roc_and_auc(y_test, y_prob, 'RF with ED - Random Train-Test Split')
...
def get_random_forest(new_zs=None):
    h5n1_us_data_df_locations_only = us_h5n1_cases_df[['latitude', 'longitude']]
    h5n1_us_data_df_locations_only_modified = modify_locations(h5n1_us_data_df_locations_only, bio_us_data_df)
    h5n1_us_data_df_locations_only_modified['observed'] = 1
    ...
    bio_us_pca_data_df_with_hn51_obs = pd.merge(bio_us_data_df, h5n1_us_data_df_locations_only_modified,
                                                on=['latitude', 'longitude'], how='outer')
    # merge bio_us_data with us poultry locations data
    bio_us_pca_data_df_with_hn51_obs_and_poultry_obs = pd.merge(bio_us_pca_data_df_with_hn51_obs,
                                                                poultry_locations_df_locations_modified,
                                                                on=['latitude', 'longitude'], how='outer')
    bio_us_pca_data_df_with_hn51_obs_and_poultry_obs = bio_us_pca_data_df_with_hn51_obs_and_poultry_obs.fillna(0)
    # add distance_to_nearest_poultry column
    add_distance_to_poultry_coln(bio_us_pca_data_df_with_hn51_obs_and_poultry_obs)
    # drop poultry_observed column
    target_bio_us_data_df_with_hn51_obs = bio_us_pca_data_df_with_hn51_obs_and_poultry_obs.drop(
    ...
    model, y_prob_all = random_forest(z_normalized_bio_us_data_df.drop(columns=['latitude', 'longitude', 'observed']),
                                                   z_normalized_bio_us_data_df['observed'])
...
#------------------------------------------------------
get_random_forest(simulated_present_samples)
