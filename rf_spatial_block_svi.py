...
def get_random_forest_5_fold (new_zs=None, n_splits= 5):
    #group_kfold = GroupKFold(n_splits=n_splits)
    fold_acc = []
    fold_auc = []
    curves_auc = []
...
#--------------------------------------------------------------
fold_acc, fold_auc, curves_auc = get_random_forest_5_fold (simulated_present_samples)
...
roc_5_fold (fold_roc_data, fold_auc, model_name='RF with SVI - 5-Fold Spatial Cross-Validation')
...
def get_random_forest(new_zs=None): 
    ...
    test_dummy = spatial_groups[spatial_groups.index.isin([test_idx])]
    ...
    #----------------------------------------
    dummy = z_normalized_bio_us_data_df_pred_true[['latitude','longitude','probability']]
    dummy = dummy.drop_duplicates()
    dummy = dummy.groupby(by=['latitude','longitude']).mean()
    dummy = dummy.reset_index(drop=False)
    show_data_on_map (dummy, color='probability')
